...
export function baseFontSize(): number {
  return parseInt(getComputedStyle(document.getElementsByTagName('html')[0]).fontSize.slice(0, -2));
}

export function setTransform({top, left, width, height}: Position, sizeMetric: ?string): Object {
  switch (sizeMetric) {
    case 'rem': {
      left = left / baseFontSize() + 'rem';
      top = top / baseFontSize() + 'rem';
      width = width / baseFontSize() + 'rem';
      height = height / baseFontSize() + 'rem';
      break;
    }
    case 'px':
    default: {
...
export function setTopLeft({top, left, width, height}: Position, sizeMetric: ?string): Object {
  switch (sizeMetric) {
    case 'rem': {
      left = left / baseFontSize() + 'rem';
      top = top / baseFontSize() + 'rem';
